...
USERNAME = "carranza"
PASSWORD = "Fairview103!"
ROUTINE_FILE_PATH = 'G:\\My Drive\\FF\\Fairview FD\\Python Schedule\\HQ and Sta2 routine.docx'
MEMBER_LIST_PATH = 'G:\\My Drive\\FF\\Fairview FD\\Python Schedule\\Member_List.csv'


# --- 2. DATE LOGIC ---
now = datetime.now()
cutoff_time = time(5, 45) # 05:45 AM
...
output_filename = f"roster_view_{date_for_url}.html"

# --- 3. BIRTHDAY REMINDER PARSER (WITH SORTING) ---
birthday_reminders_html = ""
try:
    if os.path.exists(MEMBER_LIST_PATH):
        print("Checking for upcoming birthdays...")
        found_birthdays = [] # Temporary list to store sortable data
        today = now.date()
        ...
        with open(MEMBER_LIST_PATH, mode='r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                # Helper to get values case-insensitively
        ...
        if upcoming_birthdays_html_list:
            birthday_reminders_html = f"""
            <div class="birthday-reminders">
                <h3>Upcoming Birthdays (Next 3 Days)</h3>
                <ul>{''.join(upcoming_birthdays_html_list)}</ul>
            </div>
            """
    else:
        print(f"Warning: Member list file not found at '{MEMBER_LIST_PATH}'")
...
try:
    if os.path.exists(ROUTINE_FILE_PATH):
        print("Reading daily routines from Word document...")
            ...
            if current_station:
                ...
                for day in days_of_week:
                    if text.upper().startswith(day):
                        current_day = day
                        routines[current_station][current_day] = []
                        is_day_header = True
                        break
        ...
        hq_routine = '<br>'.join(hq_tasks)
        sta2_routine = '<br>'.join(sta2_tasks)
        daily_routines_html = f"""
        <div class="daily-routines"><h3>Daily Routines for {now.strftime('%A')}</h3><div class="split-container">
            <div class="split-column"><h4>HQ</h4><p>{hq_routine or 'No tasks.'}</p></div>
            <div class="split-column"><h4>Station 2</h4><p>{sta2_routine or 'No tasks.'}</p></div>
        </div></div>"""
    else:
        print(f"Warning: Routine file not found at '{ROUTINE_FILE_PATH}'")
except Exception as e:
...
try:
    print("Fetching 3-day weather forecast for White Plains, NY...")
    ...
    if weather_response.status_code == 200:
        ...
        condition = weather_codes.get(code, "N/A")
        weather_forecast = f"☀️ {condition} | High: {high_temp}°F | Low: {low_temp}°F | Precip: {precip:.2f} in"
        
        forecast_columns = []
        for i in range(len(daily_data.get('time', []))):
            day_details = []
            day_date_obj = datetime.strptime(daily_data['time'][i], '%Y-%m-%d')
        ...
        forecast_timeline_html = f"""
        <div class="forecast-timeline">
            <h3>3-Day Forecast (White Plains, NY)</h3>
...
html_content = ""
print("Starting schedule retrieval...")
with requests.Session() as session:
    try:
        ...
        if schedule_page_response.status_code == 200:
            ...
            print("Successfully downloaded schedule data.")
        else:
            exit(f"ERROR: Failed to retrieve schedule page. Status Code: {schedule_page_response.status_code}")
    except requests.exceptions.RequestException as e:
        exit(f"An error occurred: {e}")

# --- 7. PARSE AND BUILD THE HTML REPORT ---
if html_content:
    print(f"Parsing data and creating HTML report: {output_filename}...")
    ...
    if work_schedule_header and work_schedule_header.find_next_sibling('table'):
        ...
        for row in table.find('tbody').find_all('tr'):
            ...
            if schedule_cell:
                current_schedule_name = schedule_cell.text.strip()
                if current_schedule_name not in schedules: schedules[current_schedule_name] = []
    ...
    overtime_items = [{'shift': get_shift(item['start']), 'text': item['member']} for item in schedules.get('Non Shift Overtime / Work Detail', [])]
    ...
    <!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>FD Roster</title><style>
        ...
        .daily-routines .split-container {{ display: flex; }}
        .daily-routines .split-column {{ flex: 1; padding: 0 15px; }}
        ...
        .container, .section-container {{ display: flex; flex-wrap: wrap; padding: 10px; }}
        ...
        .column h2, .section h2 {{ background-color: #34495e; color: white; padding: 15px; margin: 0; font-size: 16px; }}
        .column ul, .split-column ul {{ list-style-type: none; padding: 0; margin: 0; }}
        .column li, .split-column li {{ padding: 12px 20px; border-bottom: 1px solid #eee; }}
        .column li:last-child, .split-column li:last-child {{ border-bottom: none; }}
        .column li strong {{ color: #2980b9; }}
    </style></head><body>
        ...
        <div class="container">
            <div class="column"><h2>Dayshift (07:45 - 17:45)</h2><ul>
                <li><strong>Officers:</strong><br>{generate_list_html([item['member'] for item in schedules.get('Officers - Dayshift', [])])}</li>
                <li><strong>Firefighters:</strong><br>{generate_list_html([item['member'] for item in schedules.get('Firefighters - Dayshift', [])])}</li>
                {'<li><strong>Overtime / Work Detail:</strong><br>' + generate_list_html([item['text'] for item in overtime_items if item['shift'] == 'day']) + '</li>' if any(item['shift'] == 'day' for item in overtime_items) else ''}
                {'<li><strong>Trades:</strong><br>' + generate_list_html([item['text'] for item in trades if item['shift'] == 'day']) + '</li>' if any(item['shift'] == 'day' for item in trades) else ''}
                {'<li><strong>Time Off:</strong><br>' + generate_list_html([item['text'] for item in time_off if item['shift'] == 'day']) + '</li>' if any(item['shift'] == 'day' for item in time_off) else ''}
                {'<li><strong>Events:</strong><br>' + generate_list_html([item['text'] for item in events if item['shift'] == 'day']) + '</li>' if any(item['shift'] == 'day' for item in events) else ''}
            </ul></div>
            <div class="column"><h2>Nightshift (17:45 - 07:45)</h2><ul>
                <li><strong>Officers:</strong><br>{generate_list_html([item['member'] for item in schedules.get('Officers - Nightshift', [])])}</li>
                <li><strong>Firefighters:</strong><br>{generate_list_html([item['member'] for item in schedules.get('Firefighters - Nightshift', [])])}</li>
                {'<li><strong>Overtime / Work Detail:</strong><br>' + generate_list_html([item['text'] for item in overtime_items if item['shift'] == 'night']) + '</li>' if any(item['shift'] == 'night' for item in overtime_items) else ''}
                {'<li><strong>Trades:</strong><br>' + generate_list_html([item['text'] for item in trades if item['shift'] == 'night']) + '</li>' if any(item['shift'] == 'night' for item in trades) else ''}
                {'<li><strong>Time Off:</strong><br>' + generate_list_html([item['text'] for item in time_off if item['shift'] == 'night']) + '</li>' if any(item['shift'] == 'night' for item in time_off) else ''}
                {'<li><strong>Events:</strong><br>' + generate_list_html([item['text'] for item in events if item['shift'] == 'night']) + '</li>' if any(item['shift'] == 'night' for item in events) else ''}
            </ul></div>
        </div>
    </body></html>"""
